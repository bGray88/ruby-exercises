RSpec.describe 'max and min by pattern' do
  ...
  it 'test 5' do
    ...
    most_digits = stats[0]
    stats.each do |stat|
      most_digits = stat if stat.to_s.delete(".-").length > \
        most_digits.to_s.delete(".-").length
    end
  ...
  it 'test 6' do
    ...
    stats.each do |key, value|
      most_digits = value if value.to_s.delete(".-").length > \
        most_digits.to_s.delete(".-").length
    end
    expect(most_digits).to eq(0.27601)
  end
  ...
  it 'test 10' do
    ...
    programmers.each do |key, value|
      p programmers[key].length
      if value.length < programmers[fewest_programmers].length
