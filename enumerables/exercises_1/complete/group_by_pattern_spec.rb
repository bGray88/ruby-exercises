RSpec.describe 'group by pattern' do
  ...
  it 'groups by odds and evens' do
    ...
    numbers.each do |number|
      number % 2 == 0 ? odd_and_even[0] << number : odd_and_even[1] << number
    end
  ...
  it 'groups by first letter' do
    ...
    words.each do |string|
      case string[0]
      when "a"
        words_by_first_letter["a"] << string
      when "b"
        words_by_first_letter["b"] << string
      when "c"
        words_by_first_letter["c"] << string
      end
    end
  ...
  it 'group by uniqueness' do
    ...
    grouped = Hash.new {|hash, key| hash[key] = []}
    words.each do |string|
      grouped[string.downcase] << string
    end
  ...
  it 'group by order of magnitude' do
    ...
    grouped = Hash.new {|hash, key| hash[key] = []}
    numbers.each do |number|
      grouped[number.to_s[0..-1].length] << number
    end
